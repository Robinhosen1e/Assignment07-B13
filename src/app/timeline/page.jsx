...
import { Phone, MessageCircle, Video, ChevronDown } from "lucide-react";
...
const TimelinePage = () => {
  ...
  const filtered =
    filter === "all"
      ? timeline
      : timeline.filter((items) => items.type === filter);
            ...
            >
              ...
              <div>
                <p className="text-sm text-gray-800">
                  <span className="font-bold">{getLabel(items.type)}</span>{" "}
                  <span className="text-gray-500">
                    with {parseText(items.text)}
                  </span>
                </p>

                <p className="text-xs text-gray-400 mt-0.5">
                  {new Date(items.time).toLocaleDateString("en-US", {
                    month: "long",
                    day: "numeric",
                    year: "numeric",
                  })}
